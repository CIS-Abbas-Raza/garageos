...
const packages = [
  { tag: 'Oil, filters, checks', title: 'Quick Service', daily: '$39', monthly: '$699', items: ['Multi-point inspection', 'Fluid top-ups', 'OBD-II scan'] },
  { tag: 'Every 30k mi / 50k km', title: 'Major Service', daily: '$189', monthly: '$3,199', items: ['Full fluid change', 'Belts & filters', 'Brake & tyre report'] },
  { tag: 'Buy with confidence', title: 'Pre-Purchase Inspection', daily: '$129', monthly: '$2,199', items: ['150-point report', 'Diagnostic scan', 'Test drive notes'] },
] as const
...
export function ServicePackages() {
  const [billing, setBilling] = useState<'daily' | 'monthly'>('daily')

  return <section id="packages" className="scroll-mt-20 bg-white px-5 py-24 lg:px-8"><div className="mx-auto max-w-7xl"><SectionIntro label="Service Packages" title="Ready-to-use service menus for your shop." body="Launch on day one with pre-built packages you can customize per branch." /><div className="mt-8 flex flex-col items-center gap-2"><div className="inline-flex rounded-full border border-landing-line bg-[#f8fafc] p-1" role="group" aria-label="Package billing frequency"><button type="button" aria-pressed={billing === 'daily'} onClick={() => setBilling('daily')} className={`rounded-full px-5 py-2 text-sm font-semibold transition-colors ${billing === 'daily' ? 'bg-landing-blue text-white shadow-sm' : 'text-landing-muted hover:text-landing-foreground'}`}>Daily</button><button type="button" aria-pressed={billing === 'monthly'} onClick={() => setBilling('monthly')} className={`rounded-full px-5 py-2 text-sm font-semibold transition-colors ${billing === 'monthly' ? 'bg-landing-blue text-white shadow-sm' : 'text-landing-muted hover:text-landing-foreground'}`}>Monthly</button></div><span className={`min-h-5 text-xs font-semibold text-landing-green transition-opacity ${billing === 'monthly' ? 'opacity-100' : 'opacity-0'}`}>Save up to 18%</span></div><div className="mt-8 grid gap-5 lg:grid-cols-3">{packages.map((pkg) => <div key={pkg.title} className="rounded-2xl border border-landing-line bg-white p-7 shadow-sm"><p className="text-sm text-landing-muted">{pkg.tag}</p><h3 className="mt-5 text-2xl font-bold text-landing-foreground">{pkg.title}</h3><p key={billing} className="mt-4 animate-in text-3xl font-bold text-landing-foreground duration-300 fade-in zoom-in-95">Starting at {billing === 'daily' ? pkg.daily : pkg.monthly}</p><ul className="mt-7 space-y-3">{pkg.items.map(item => <li key={item} className="flex items-center gap-3 text-sm text-landing-muted"><Check className="size-4 text-landing-green" />{item}</li>)}</ul></div>)}</div></div></section>
}
