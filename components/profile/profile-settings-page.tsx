...
export function ProfileSettingsPage() {
  ...
  const [profile, setProfile] = useState({ name: '', email: '', phone: '', address: '', country: 'United States', state: '', photo: '' })
  ...
  useEffect(() => {
    if (!user) return
    setProfile({ name: user.userName, email: user.email, phone: user.phoneNumber ?? '', address: user.address ?? '', country: user.country ?? 'United States', state: user.state ?? '', photo: user.profilePhoto ?? '' })
  }, [user])

  const updateField = (field: keyof typeof profile, value: string) => setProfile((current) => ({ ...current, [field]: value }))
  const updateState = (value: string) => setProfile((current) => ({ ...current, state: value }))
  ...
  const savePassword = async (event: React.FormEvent) => {
    event.preventDefault()
    if (!password.next || !password.confirm) return toast.error('Complete all password fields.')
    ...
    try { await changePassword(password.next); setPassword({ next: '', confirm: '' }); toast.success('Password updated successfully.') } catch (error) { toast.error(error instanceof Error ? error.message : 'Unable to update your password.') } finally { setSavingPassword(false) }
  }
