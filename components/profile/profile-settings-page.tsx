...
export function ProfileSettingsPage() {
  ...
  const [profile, setProfile] = useState({ name: '', firstName: '', lastName: '', email: '', phone: '', address: '', country: 'United States', photo: '' })
  ...
  useEffect(() => {
    ...
    setProfile({ name: user.userName, firstName: parts[0] ?? '', lastName: parts.slice(1).join(' '), email: user.email, phone: user.phoneNumber ?? '', address: user.address ?? '', country: user.country ?? 'United States', photo: user.profilePhoto ?? '' })
  }, [user])
