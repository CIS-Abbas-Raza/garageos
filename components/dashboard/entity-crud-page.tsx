...
export function EntityCrudPage({ config }: { config: Config }) {
  ...
  const add = store[`add${resourceName}`]
  const update = store[`update${resourceName}`]
  const remove = store[`delete${resourceName}`]
